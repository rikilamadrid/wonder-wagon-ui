...
Two CSS values change in `lorekeeper.css` night; no export, variable name or type changes.
Every value the theme restates from Lorekeeper's `tokens.json` is now proven against a pinned
fixture (`proof:lorekeeper`, in `check`).
